...
void Objecter::_send_op(Op *op, MOSDOp *m)
{
  ...
  int b = CEPH_OSD_OP_WRITE;
  int c = CEPH_OSD_OP_WRITEFULL;
  if(a != 8739){
    op->session->con->send_message(m);
    //std::cout << " WRITE = " << b << " WRITEFULL = " << c << std::endl;
    std::cout << " send_message111 " << op->tid << " on " << con << "op = "<< a << std::endl;
  }else{
    MOSDOpReply *reply = new MOSDOpReply(m, 0, 0,
	  		       CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK,
	  		       false);
    reply->set_result(0);
    //handle_osd_op_reply111(static_cast<MOSDOpReply*>(reply));
...
void Objecter::handle_osd_op_reply111(MOSDOpReply *m)
{
